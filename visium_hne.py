...
planar_spatial_connectivities, planar_spatial_distances = sq.gr.spatial_neighbors(adata, delaunay=True, copy=True)
...
# print(adata.uns['spatial']) # grid coords?
# print(adata.obsm['spatial']) # spatial coordinates
...
adj_2d = np.column_stack((adj[0], adj[1]))

# gift wrapping -> convex hull
# TODO: go over edges of current hull_point to find the next point
convex_hull = []
...
fig.savefig('./figures/visium_hne/spatial_coordinates.svg', format='svg')
...
fig.savefig('./figures/visium_hne/spatial_distances_to_hull.svg', format='svg')
